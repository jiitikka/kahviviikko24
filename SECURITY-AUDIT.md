...
Date: 2026-08-01. Scope: the whole repository at commit `381f30b`, plus its
...
### 4. Vulnerable `postcss` in the direct dev dependency — Low — fixed

The declared `postcss` was `^8.5.6`, matched by three advisories
(GHSA-qx2v-qp2m-jg93 XSS via unescaped `</style>`, GHSA-6g55-p6wh-862q and
GHSA-r28c-9q8g-f849, both arbitrary file read via `sourceMappingURL`). Build-time
only, operating on this repo's own CSS, hence Low. Raised to `^8.5.18`, which
resolves to 8.5.25 and clears all three.
...
- **`postcss` 8.4.31 bundled inside `next`** — same three advisories as finding
  4, but vendored under `node_modules/next/node_modules/` where this project
  cannot override it, and no fixed `next` release exists yet
  (all versions through 16.3.0-preview.7 are affected). Build-time only, and it
  only ever parses this repository's own CSS. Re-check on each `next` upgrade.
